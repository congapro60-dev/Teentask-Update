        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          >
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-xl font-black text-gray-900">{t('nameChangeRequest')}</h3>
            ...
            <form onSubmit={handleNameChangeSubmit} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-6 space-y-6 overflow-y-auto no-scrollbar">
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t('currentDisplayName')}</label>
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t('newDisplayName')}</label>
                  <input
                    type="text"
                    required
                    value={nameChangeData.newName}
                    onChange={(e) => setNameChangeData({ ...nameChangeData, newName: e.target.value })}
                    placeholder={t('enterNewDisplayName')}
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t('reasonForChange')}</label>
                  <textarea
                    required
                    value={nameChangeData.reason}
                    onChange={(e) => setNameChangeData({ ...nameChangeData, reason: e.target.value })}
                    placeholder={t('explainReasonForChange')}
                ...
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t('verificationProof')}</label>
                    ...
                    <div className="flex items-center justify-center w-full">
                      <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 rounded-3xl cursor-pointer hover:bg-gray-50 transition-all">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          ...
                          <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">{t('uploadPhoto')}</p>
                        </div>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100">
                ...
                >
                  {submittingNameChange ? t('submitting').toUpperCase() : t('submitRequest')}
                </button>
              </div>
            </form>
          </motion.div>
